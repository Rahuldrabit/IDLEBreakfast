                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try {
                            ...
                            try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/idlecenter", "root", "my1234sl");
                                 PreparedStatement stmt = con.prepareStatement("SELECT * FROM userinfo WHERE username = ?")) {
                                stmt.setString(1, user);
                                try (ResultSet rs = stmt.executeQuery()) {
                                    if (rs.next()) {
                                        JOptionPane.showMessageDialog(null, "User found: " + user);
                                    } else {
                                        JOptionPane.showMessageDialog(null, "No user found for " + user);
                                    }
                                }
                            }
                        } catch (Exception ex) {
