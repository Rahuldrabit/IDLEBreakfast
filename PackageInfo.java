...
public class PackageInfo extends JFrame {
	...
	public PackageInfo() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try {
                            ...
                            try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/idlecenter", "root", "my1234sl");
                                 PreparedStatement stmt = con.prepareStatement("SELECT * FROM packageinfo WHERE username = ?")) {
                                stmt.setString(1, user);
                                try (ResultSet rs = stmt.executeQuery()) {
                                    if (rs.next()) {
                                        JOptionPane.showMessageDialog(null, "Package info found for " + user);
                                    } else {
                                        JOptionPane.showMessageDialog(null, "No package info found for " + user);
                                    }
                                }
                            }
                        } catch (Exception ex) {
		...
		panel_1.setBounds(0, 102, 626, 372);
		...
		JLabel lblNewLabel_3 = new JLabel("Dear Customer, You ordered above Item for a ");
		lblNewLabel_3.setBounds(137, 316, 250, 37);
		panel_1.add(lblNewLabel_3);
		
		JLabel lblNewLabel_4 = new JLabel("Week");
		lblNewLabel_4.setBounds(412, 316, 65, 37);
